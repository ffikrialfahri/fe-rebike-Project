...
const ResourceTable = ({
  title,
  // Props untuk data statis
  staticData,
  loading: loadingProp,
  error: errorProp,
  // Props untuk data dari Redux
  fetchDataAction,
  dataSelector,
  loadingSelector,
  errorSelector,
  ...
  // Gunakan selector HANYA jika data tidak disediakan secara statis
  const reduxData = dataSelector ? useSelector(dataSelector) : [];
  const reduxLoading = loadingSelector ? useSelector(loadingSelector) : false;
  const reduxError = errorSelector ? useSelector(errorSelector) : null;
  const serverPagination = useSelector((state) => state.admin.pagination);

  // Tentukan sumber data dan status yang akan digunakan oleh komponen
  const rawData = staticData !== undefined ? staticData : reduxData;
  const loading = loadingProp !== undefined ? loadingProp : reduxLoading;
  const error = errorProp !== undefined ? errorProp : reduxError;
  ...
  useEffect(() => {
    // Fetch data hanya jika tidak ada data statis dan ada action yang disediakan
    if (!staticData && fetchDataAction) {
      const params = { page: currentPage, size: itemsPerPage };
      if (searchTerm) params.id = searchTerm; // Asumsi search via ID/term
      dispatch(fetchDataAction(params));
    }
  }, [dispatch, fetchDataAction, currentPage, searchTerm, staticData]);
  ...
  const handlePageClick = (page) => setCurrentPage(page - 1);
  ...
  const generatePageNumbers = () => {
    ...
    const startPage = Math.max(2, currentPageNum - siblingCount);
  ...
  const filteredData = React.useMemo(() => {
    ...
    if (enableStatusFilter && selectedStatus !== 'Semua') {
      result = result.filter(item => {
        ...
        if (statusColumn?.accessor) {
          return statusColumn.accessor(item) === selectedStatus;
        }
        return false;
      });
    }
        ...
        <table className="min-w-full leading-normal">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={tableColumns.length} className="text-center py-10 text-red-500">Error: {typeof error === 'object' ? JSON.stringify(error) : error}</td>
              </tr>
          ...
          >
              ...
              <div key={index} className="flex items-center">
                ...
                {page === '...' ? (
                  ...
                  <button
                    ...
                    aria-current={currentPagination.page + 1 === page ? 'page' : undefined}
                  >
                    {page}
                  </button>
                )}
            ...
            <div className='flex items-center'>
              ...
              <button
                ...
                disabled={!currentPagination.totalPages || currentPagination.page + 1 >= currentPagination.totalPages}
                className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <span>Next</span>
